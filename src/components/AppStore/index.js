...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const searchResults = filteredApps.filter(eachFilteredApp =>
      eachFilteredApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
        ...
        <div className="app-store-container">
          ...
          <ul className="apps-container">
            {filteredApps.map(eachApp => (
              <AppItem key={eachApp.appId} eachApp={eachApp} />
            ))}
          </ul>
        </div>
      </div>
